...
void load(char** names, char** surnames, char** patronymics, string sex)
{
	...
	if (Surnames.is_open())
	{
		...
		int i = 0;
		while (Surnames.getline(line, 100))
		{
			surnames[i] = line;
			i++;
		}
	...
	if (Patronymics.is_open())
	{
		...
		int i = 0;
		while (Patronymics.getline(line, 100))
		{
			patronymics[i] = line;
			i++;
		}
